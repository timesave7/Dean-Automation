"""
Dean_Brokerage TQQQ 투자 신호 - Crash Fund 전략 R3 + QQQI (V2.4)

Brokerage 계좌 전용 - 세금 고려 사항:
- TRANSFER 매도 시: LTCG 18.8% (15% LTCG + 3.8% NIIT) 발생
- QQQI 배당: 95.8% ROC (즉시 비과세, cost basis 차감) + 4.2% Section 1256 (60/40)
- ROC 효과로 SCHD 대비 세금 마찰 ~10배 적음
================================================================
...
# ─────────────────────────────────────────────
STATE_FILE = "Dean_Brokerage_state.json"
...
UPDATE_WORKFLOW    = "Dean_Brokerage_update_cost_basis_04_23_2026.yml"
...
def fetch_history(symbol, period, required=True, tries=4, min_rows=1):
    ...
    기존 코드는 `hist.empty` 만 검사했는데, Yahoo가 레이트리밋되거나 응답이
    부분적으로 깨지면 yfinance 는 '행은 있고 Close 가 전부 NaN' 인 DataFrame 을
    돌려준다. empty 는 False 라 방어망을 통과하고, 이후 산술이 전부 nan 으로
    전파되어 2026-09-11 처럼 $nan 메일이 발송된다. 그래서 여기서 NaN 을 직접 검사한다.
    """
    last_err = "원인 미상"
    for attempt in range(1, tries + 1):
        try:
            ...
            if hist is None or hist.empty:
                ...
                last_err = f"Close 컬럼 없음 (columns={list(hist.columns)})"
            else:
...
def get_signals():
    ...
    # 배당ETF 현재가 (QQQI) — NaN 은 truthy 이므로 pd.notna 로 검사해야 한다
    div_price = None
    ...
    if s['crash_3_hit']:
        ...
        <div style="margin-top:16px;padding:14px 18px;background:#f0faf4;
            ...
            <p style="margin:0 0 10px;font-size:13px;color:#1e8449;font-weight:bold;">
                ⚠️ 거래 체결 후 COST_BASIS를 업데이트하세요 (새 CB = 체결가)<br>💰 Brokerage 세금: LTCG 18.8% 발생 (다음 거래일 정도 자동 차감)
            </p>
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;
    ...
    <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:white;
        ...
        <h1 style="margin:0;font-size:20px;">📊 Dean - Brokerage TQQQ 투자 신호</h1>
...
def send_email(html_content, s):
    ...
    subject = f"[Dean - Brokerage TQQQ투자신호] {signal} | TQQQ ${s['tqqq_price']} | {s['date']}"
