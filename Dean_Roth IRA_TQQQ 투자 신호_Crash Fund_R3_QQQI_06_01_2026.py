"""
Dean_Roth IRA TQQQ 투자 신호 - Crash Fund 전략 R3 + QQQI (V2.4)
================================================================
...
- 2026-06-01: 전환 분배 자동판정 추가 (S3 룰) - 50:50 하드코딩 제거.
              현금 비중(cash_pct)에 따라 100% QQQI vs 50:50 자동 결정.

핵심 룰:
- 트리거: TQQQ 가격 ≥ CB × 1.30 (CB = 직전 매도가/체결가로 리셋된 값, 최초 원가 아님)
- 매도: 수익분(현재가-CB)의 30%를 전환. 분배는 현금 비중에 따라 (S3 룰):
    · 현금 ≥ 포트 5%  → 수익분 30% 전액 QQQI (QQQI 집중)
    · 현금 <  포트 5%  → QQQI 15% + 현금 15% (50:50, 현금 우선 재건)
- Crash Hunter L1/L2/L3: ATH -55%/-70%/-80%
- 매수자금 한도: 포트폴리오 40%

state.json 기반 동적 로딩:
- cost_basis, trigger_pct, transfer_pct, div_etf, rule_version, cash_pct
"""
...
# ─────────────────────────────────────────────
STATE_FILE = "Dean_Roth IRA_state.json"
...
DEFAULT_CASH_PCT    = None           # 순수 현금 비중(포트 대비). None = 미확인 → 양쪽 안내

CASH_THRESHOLD      = 0.05           # S3 룰 기준: 현금 ≥ 5% → 100% QQQI / < 5% → 50:50
    ...
    if os.path.exists(STATE_FILE):
        try:
            ...
            for k in ['cost_basis', 'trigger_pct', 'transfer_pct',
                      'div_etf', 'rule_version', 'cash_pct', 'last_updated']:
                if k in loaded:
                    state[k] = loaded[k]
            ...
            _cp = state['cash_pct']
            print(f"   CASH_PCT   = {('%.0f%%' % (_cp*100)) if _cp is not None else '미확인(양쪽 안내)'}")
...
CASH_PCT      = _STATE['cash_pct']       # 순수 현금 비중(포트 대비). None = 미확인

MAX_DIV_PCT   = 0.40   # 매수자금 최대 비중 40% (현금 + QQQI 합산)


# ─────────────────────────────────────────────
# 전환 분배 자동판정 (S3 룰) - 50:50 하드코딩 대체
# ─────────────────────────────────────────────
def transfer_plan(cash_pct=CASH_PCT, transfer_pct=TRANSFER_PCT, div_etf=DIV_ETF):
    """
    현금 비중에 따라 수익분 전환 분배를 결정한다 (전략문서 S3 / P2 룰).
      · 현금 ≥ 5%  → 수익분 전액 QQQI (QQQI 집중)
      · 현금 <  5%  → QQQI 절반 + 현금 절반 (50:50, 현금 우선 재건)
      · 미확인     → 양쪽 모두 안내 (Dean이 현금 비중 보고 선택)
    반환: (요약문구, 모드)  모드 ∈ {'qqqi100','split','unknown'}
    """
    t = transfer_pct * 100
    if cash_pct is None:
        return (f"전환액 = 수익분 × {t:.0f}% = (현재가 − CB) × 보유주식수 × {t/100:.2f}. "
                f"분배는 현재 순수 현금 비중으로 결정 → "
                f"[현금 ≥ 포트 5%] 전액 {div_etf} ／ "
                f"[현금 < 5%] {div_etf} 절반 + 현금 절반 (50:50, 현금 우선 재건)",
                'unknown')
    if cash_pct >= CASH_THRESHOLD:
        return (f"수익분 {t:.0f}% 전액 {div_etf} 전환 "
                f"(현금 {cash_pct*100:.0f}% ≥ 5% → QQQI 집중)", 'qqqi100')
    return (f"수익분 {t:.0f}%를 {div_etf} {t/2:.0f}% + 현금 {t/2:.0f}% 50:50 분할 "
            f"(현금 {cash_pct*100:.0f}% < 5% → 현금 우선 재건)", 'split')
...
UPDATE_WORKFLOW    = "Dean_Roth IRA_update_cost_basis.yml"
...
def build_signal_html(s):
    ...
    if s['crash_3_hit']:
        ...
        _plan_text, _plan_mode = transfer_plan()
        action_lines = [
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;
    ...
    <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:white;
        ...
        <h1 style="margin:0;font-size:20px;">📊 Dean - Roth IRA TQQQ 투자 신호</h1>
...
def print_signal(s):
    LINE = "=" * 58
    print(f"\n{LINE}")
    print(f"  💰 Dean's TQQQ {RULE_VERSION} + {DIV_ETF} 신호  |  {s['date']}")
...
def send_email(html_content, s):
    ...
    subject = f"[Dean - Roth IRA TQQQ투자신호] {signal} | TQQQ ${s['tqqq_price']} | {s['date']}"
